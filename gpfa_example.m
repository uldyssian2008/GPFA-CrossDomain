...
optTime = 5; % number of EM iteration
C = randn(q,p); scale = abs(randn(p,1)) + 10^(-1); R = diag(abs(randn(q,1)) + 0.01); d = randn(q,1); % parameter initialization

for t = 1:optTime   
    [Mean,Cov,Cov2] = EMexp(C,scale,R,d,Y);
    [C,scale,R,d] = EMmax(Mean,Cov,Cov2,Y);
